...
int get_student(int fd, int id, student_t *s)
{
    ...
    if(lseek(fd, myoffset, SEEK_SET) == -1){
		return ERR_DB_FILE;
    }

    ssize_t bytes_read = read(fd, s, STUDENT_RECORD_SIZE);
    if(memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0 || bytes_read == 0 || s->id != id){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if (read(fd, &mystudent, STUDENT_RECORD_SIZE) < 0)
    {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
...
validate_range(id, gpa) *  write an empty student record - see EMPTY_STUDENT_RECORD from db.h at
 *  that location.
 *
...
{
    student_t target_student;
    int result = get_student(fd, id, &target_student);
...
int count_db_records(int fd)
{
	...
	while ((bytes_read = read(fd, &current, sizeof(student_t))) > 0) {
        if (memcmp(&current, &empty, sizeof(student_t)) != 0) {
            record_count++;
        }
    }
...
int print_db(int fd)
{
	...
	int first_valid = 0;
    while (read(fd, &current, sizeof(student_t)) == sizeof(student_t)) {
        if (memcmp(&current, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0) {
            continue;
        }
        ...
        float gpa = current.gpa / 100.0;
        printf(STUDENT_PRINT_FMT_STRING, current.id, current.fname, current.lname, gpa);
        record_count++;
    }
